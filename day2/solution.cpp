...
bool check_invalid_id_2(string id) {
    for (int window_len = 1; window_len < id.length(); window_len++) {
        

        if (id.length() % window_len != 0) {
            continue;
        }

        string substring = id.substr(0, window_len);

        int repetitions = 1;

        bool isRepeated = true;

        while (window_len * (repetitions + 1) <= id.length()) {
            int start_index = window_len * (repetitions);
            string temp_substring = id.substr(start_index, window_len);
...
void get_invalid_id_2(string input_file)
{
    ...
    while (getline(id_ranges, id_range, ','))
    {
        ...
        for (long long id = stoll(first_id); id <= stoll(last_id); id++) {
            ...
            if (check_invalid_id_2(id_string)) {
...
int main()
{
    ...
    //get_invalid_id_2(sample_input);
    get_invalid_id_2(input);
}